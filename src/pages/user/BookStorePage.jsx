...
const BookStorePage = () => {
    ...
    // Thêm state này để kiểm soát render danh sách sách
    ...
    // Khi location.search đổi, cập nhật filter và fetch sách ngay sau khi cập nhật filter
    useEffect(() => {
        ...
        dispatch(resetBooks()); // Reset danh sách sách ngay khi filter đổi
        setFilters((prev) => {
            const newFilters = {
                ...prev,
                category,
            ...
            fetchBooksWithFilters(newFilters);
            return newFilters;
        });
        setCurrentPage(1);
        setIsFilterReady(true); // Đánh dấu đã đồng bộ filter
    }, [location.search]);

    // Hàm fetchBooks mới nhận filter làm tham số
    const fetchBooksWithFilters = useCallback((customFilters) => {
        const params = new URLSearchParams(location.search);
        const query = params.get('q') || '';
        const filtersToUse = customFilters || filters;
        const fetchParams = {
            q: query || undefined,
            category: filtersToUse.category.length > 0 ? filtersToUse.category.join(',') : undefined,
            author: filtersToUse.author.length > 0 ? filtersToUse.author.join(',') : undefined,
            publisher: filtersToUse.publisher.length > 0 ? filtersToUse.publisher.join(',') : undefined,
            minPrice: filtersToUse.priceRange.min > 0 ? filtersToUse.priceRange.min : undefined,
            maxPrice: filtersToUse.priceRange.max !== Infinity ? filtersToUse.priceRange.max : undefined,
            available: filtersToUse.availability.length ? filtersToUse.availability[0] : undefined,
    ...
    // Khi filters hoặc currentPage đổi (không phải do location.search), fetch sách
    useEffect(() => {
        // Nếu location.search vừa đổi thì đã fetch ở useEffect trên rồi, tránh fetch 2 lần
        // Chỉ fetch khi filters hoặc currentPage đổi do thao tác người dùng
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-12">
            <div className="container mx-auto px-4">
                ...
                {/* Chỉ render danh sách sách khi filter đã sẵn sàng */}
                {isFilterReady ? (
